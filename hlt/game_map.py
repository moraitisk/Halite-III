...
class GameMap:
    ...
    def nearest_base(self, ship, me):
        """
        Returns the nearest base for a ship to deposit halite.

        :return: The position of the nearest base and it's distance.
        """
        dropoffs = me.get_dropoffs()
        nearest = (me.shipyard.position, self.calculate_distance(ship.position, me.shipyard.position))
        for base in dropoffs:
            base_dst = self.calculate_distance(ship.position, base.position)
            if base_dst < nearest[1]: nearest = (base.position, base_dst)
